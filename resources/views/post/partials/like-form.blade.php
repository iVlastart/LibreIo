...
<x-card class="flex flex-row items-center justify-between gap-2">
    {{-- download --}}
    {{-- like --}}
    <form method="post" action="/like" class="like-form" title="like">
        ...
        <button id="btnLike" class="span border bg-transparent border-transparent hover:cursor-pointer">
            <span class="flex flex-row gap-2">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" stroke-width="1" stroke="currentColor" class="size-7 text-blue-500">
                ...
                <span class="number like-count {{ $isLiked ? "liked" : "" }}">{{$likeCount}}</span>
            </span>
        </button>
    </form>
    {{-- divider --}}
    ...
    <form action="/like" method="post" class="dislike-form" title="dislike">
        ...
        <button id="btnDislike" class="span border bg-transparent border-transparent hover:cursor-pointer">
            <span class="flex flex-row gap-2">
                ...
                <span class="number dislike-count {{ $isDisliked ? "disliked" : "" }} pr-2">{{$dislikeCount}}</span>
            </span>
        </button>
    </form>
</x-card>